...
func main() {
	...
	fmt.Printf("Return after evaluating Z=Model(X: 1, Y: 2, W: 3): %v=%v %v\n", variable, result, err)
	result, variable, err = model.Evaluate(Arguments{
		X: 1, Y: 2, Z: 3,
	}.Wrap(), nil)
	...
	fmt.Printf("Return after evaluating X=Model^-1(W: 1, Y: 2, Z: 3): %v=%v %v\n", variable, result, err)
	result, variable, err = model.Evaluate(Arguments{
		X: 1, W: 2, Z: 3,
	}.Wrap(), nrSolver)
	...
	fmt.Printf("Return after evaluating Y=Model^-1(X: 1, W: 2, Z: 3): %v=%v %v\n", variable, result, err)
}